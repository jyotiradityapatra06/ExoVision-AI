...
import { Activity, ArrowLeft, BadgeCheck, BrainCircuit, CircleCheck, Database, FileText, Info, Orbit, ScanSearch, Telescope } from "lucide-react";
...
export function ResultsDashboard({ analysis }: { analysis: AnalysisResult }) {
  ...
  const parameters = [
    ...
    ["Detection", analysis.transit.detected ? "Candidate detected" : "Not detected"],
  ];
    ...
    <main className="app-workspace max-w-[1480px]">
      ...
      <header className="mission-panel data-grid relative mt-5 overflow-hidden p-6 sm:p-8 lg:p-10">
        ...
        <div className="relative z-10 grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-end">
          ...
          <div className="rounded-2xl border border-cyan-300/15 bg-[#040a16]/75 p-5 backdrop-blur-xl"><div className="flex items-start justify-between"><div><p className="telemetry-label">Model score</p><p className="mt-2 font-mono text-5xl font-semibold tracking-tight text-white">{confidence}</p></div><span className="flex h-11 w-11 items-center justify-center rounded-xl border border-cyan-300/20 bg-cyan-300/[0.08]"><BadgeCheck className="h-5 w-5 text-cyan-200" /></span></div><div className="mt-5 h-1.5 overflow-hidden rounded-full bg-white/[0.06]"><div className="h-full rounded-full bg-gradient-to-r from-cyan-500 to-cyan-200 shadow-[0_0_12px_rgba(103,232,249,.6)]" style={{ width: `${candidate ? candidate.confidence * 100 : 0}%` }} /></div><div className="mt-5 border-t border-white/[0.07] pt-4"><p className="telemetry-label">Candidate classification</p><p className="mt-2 text-lg font-semibold text-cyan-100">{candidate?.classification ?? "No candidate classified"}</p><p className="mt-2 text-xs leading-5 text-slate-500">Random Forest classifier output; not a confirmation probability.</p></div></div>
        </div>
      </header>

      <section className="mt-6"><AnalysisSummary result={analysis} /></section>

      <aside className="mt-6 flex items-start gap-3 rounded-2xl border border-violet-300/15 bg-violet-300/[0.045] p-5 text-sm leading-6 text-slate-300" aria-label="Scientific interpretation guidance"><Info className="mt-0.5 h-5 w-5 shrink-0 text-violet-300" /><div><p className="font-semibold text-white">Candidate-screening context</p><p className="mt-1 text-slate-400">ExoVision detects transit-like signals algorithmically and uses a Random Forest classifier to assist candidate assessment. Model scores are not confirmation probabilities; promising candidates require independent scientific validation.</p></div></aside>
      ...
      <section className="mt-8" aria-labelledby="observation-visualizations">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <article className="mission-panel"><div className="flex flex-col gap-3 border-b border-white/[0.08] px-5 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-6"><div><p className="telemetry-label">Figure 02 / periodic signal</p><h3 className="mt-1 font-semibold text-white">Phase-folded transit</h3><p className="mt-1 text-xs text-slate-500">Measurements aligned to the detected orbital period.</p></div><span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 font-mono text-[9px] uppercase tracking-wider ${analysis.transit.detected ? "border-emerald-300/20 bg-emerald-300/[0.07] text-emerald-200" : "border-slate-300/10 bg-white/[0.03] text-slate-500"}`}><Orbit className="h-3 w-3" /> {analysis.transit.detected ? "Transit-like signal detected" : "No signal detected"}</span></div><div className="p-3 sm:p-5"><FoldedCurveChart flux={analysis.transit.flux} phase={analysis.transit.phase} /></div><div className="grid grid-cols-3 border-t border-white/[0.06]">{scientificMetrics.map((metric) => <div className="border-r border-white/[0.06] px-4 py-3 last:border-r-0" key={metric.label}><p className="telemetry-label">{metric.label}</p><p className="mt-1 truncate font-mono text-[10px] text-slate-300">{metric.value}</p></div>)}</div></article>
        </div>
      </section>
