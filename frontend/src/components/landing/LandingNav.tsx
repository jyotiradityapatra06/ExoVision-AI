  ...
  return (
    <header className={`landing-nav ${scrolled || open ? "landing-nav-solid" : ""}`}>
      <nav aria-label="Public navigation" className="landing-shell flex h-[72px] items-center justify-between">
        ...
        <div className="hidden items-center gap-7 lg:flex">{sectionLinks.map((link) => <a className="landing-nav-link" href={link.href} key={link.label}>{link.label}</a>)}</div>
        <div className="hidden items-center gap-5 sm:flex"><Link className="landing-nav-link" href="/auth/login">Sign In</Link><Link className="landing-nav-action" href="/upload">Get Started</Link></div>
      ...
      {open && <nav aria-label="Mobile public navigation" className="border-t border-white/10 bg-[#06090d] px-5 py-5 sm:hidden"><div className="flex flex-col gap-1">{sectionLinks.map((link) => <a className="landing-mobile-link" href={link.href} key={link.label} onClick={() => setOpen(false)}>{link.label}</a>)}<Link className="landing-mobile-link" href="/auth/login">Sign In</Link><Link className="landing-button landing-button-primary mt-3" href="/upload">Get Started</Link></div></nav>}
    </header>
  );
}
